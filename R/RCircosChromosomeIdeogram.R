...
RCircos.Chromosome.Ideogram.Plot<-function(tick.interval=0,cex=1,chr.name.pos=NULL,suppressLabels=FALSE)

{
	...
	if (!suppressLabels) {
    RCircos.Label.Chromosome.Names(chr.name.pos=chr.name.pos,cex=cex)
}	
}
...
#
RCircos.Label.Chromosome.Names <- function (chr.name.pos=NULL,cex=1,
	suppressLabels=FALSE) {
    RCircos.Par  <- RCircos.Get.Plot.Parameters()
    if(is.null(chr.name.pos)) {
        chr.name.pos <- RCircos.Par$chr.name.pos;
print(chr.name.pos)
print(RCircos.Par$track.in.start)
print(RCircos.Par$track.out.start)
    } else {
	...
	tmp <- RCircos.Par$chr.name.pos
	x <- RCircos.Pos[mid, 1]*(tmp-0)
  y <- RCircos.Pos[mid, 2]*(tmp-0)
	print(sprintf("%1.2f, %1.2f\n", x,y))
        text(x=x,y=y,label=chrName, pos=ifelse(mid <= rightSide, 4, 2),
             srt=RCircos.Pos$degree[mid],cex=cex);
    }
}
